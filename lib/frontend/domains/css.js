...
import arrayFind from 'core-js/actual/array/find'
...
export function styleSheetRegistered ({ cssText, url, ownerNode }) {
    ...
    const styleSheetElement = arrayFind(
        cssStyleSheets,
        (cssStyleSheet) => cssStyleSheet.href && cssStyleSheet.href.indexOf(url) > -1
    )
...
let computedStyleUpdates = [];
let computedStyleObserver;

export function trackComputedStyleUpdates({ propertiesToTrack }) {
    const propsToTrack = propertiesToTrack;
    return {};
}

export function takeComputedStyleUpdates() {
    return { nodeIds: [] };
}
